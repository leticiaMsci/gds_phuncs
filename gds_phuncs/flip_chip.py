...
def pwb_link_trench(
    ...
    ruler_port2.mirror((1,0))
    # port2 = ruler_port2.ports[Nports]
    ...
    for ii in range(1, Nports+1):
        D.add_port(name = 100+ii, port=ruler_port1.ports[ii])
        D.add_port(name = 200+ii, port=ruler_port2.ports[ii])
...
align_box_layer=15, align_boxes_distance = 3000):
    ...
    if align_layer is not None:
        if not hasattr(align_box_layer, "__len__"):
            # if its not a list, make it a single element list
            align_box_layer = [align_box_layer]
        Delta=  0
...
def amplifier_pad(wg_w = 1.5,
    ...
    if pwb_trench_layer is not None:
        print("Trenches")
        if align_box_layer is not None:
            Align= gds.alignment_mark_ebeam(mark_layer = wg_layer, caliper_layers = [wg_layer, pwb_trench_layer, pad_layer],
                                            label="L", align_box_layer=align_box_layer)
        ...
        # gold bonding pads
        R = pg.rectangle(size=[pad_width, oxpad.ysize-gold_edge_distance], layer = bonding_layer)
        ...
        if align_box_layer is not None: 
            Align= gds.alignment_mark_ebeam(mark_layer = wg_layer, caliper_layers = [wg_layer, pad_layer], label="L", 
                        align_box_layer=align_box_layer)

        # gold bonding pads
        R = pg.rectangle(size=[pad_width, pad_width-gold_edge_distance-2*pad_wg_overlap], layer = bonding_layer)
    # MAking separation  of gold pads with 
    Rcutout = pg.rectangle(size = [gold_pitch, 2*D.ysize])
    Rcutout.rotate(angle)
    Rcutout.center = R.center
    r_gold = D<<pg.boolean(A = R, B = Rcutout, operation = 'not', precision = 1e-6,
               num_divisions = [1,1], layer = bonding_layer)
    r_gold.center = rpad.center

    # adding ruler at the bottom
    ruler_port2 = D<<RulerPort
    ruler_port2.rotate(180)
    ruler_port2.ymax = D.ymin+pwb_trench_wg_overlap
    ruler_port2.x = ruler_port1.x

    port_y_distance = ruler_port1.y-ruler_port2.y
    port_x_distance = port_y_distance*np.sin(angle*np.pi/180)

    ruler_port1.movex(-port_x_distance/2)
    ruler_port2.movex(port_x_distance/2)

    if align_box_layer is not None:   

        align1 = D<<Align
        align1.move(origin = align1.ports["c"].center, destination = (rpad.xmax+distance_alignment_marks,
                    rpad.ymin-distance_alignment_marks))
        align2 = D<<Align
