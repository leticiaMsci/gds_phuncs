...
def curved_heater(heater_L = 1000,
    ...
    Y = CrossSection()
    Y.add(width= heater_w, offset=offset_heater_wg, layer = heater_layer, ports=[1,2])
    ...
    CH<<pr.route_quad(gsg.ports["g1"], curved_heater.ports[1], layer = [heater_layer, gold_layer])
    CH<<pr.route_quad(gsg.ports["s"], curved_heater.ports[2], layer = [heater_layer, gold_layer])
    ...
    # CH.add_port(name = "heater1", port = curved_heater.ports[1])
    # CH.add_port(name = "heater2", port = curved_heater.ports[2])
